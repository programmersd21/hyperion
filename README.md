...
<img src="https://img.shields.io/badge/kernel-6.19.6--Hyperion--2.2.0-blue?style=for-the-badge&color=43a047" alt="Kernel Version">
</a>
...
```
uname -r  ->  6.19.6-Hyperion-2.2.0
uname -v  ->  #1 SMP PREEMPT Linux 6.19.6-Hyperion-2.2.0 (Soumalya Das) 2026
```
...
### v2.2.0 — USB Stability & Autosuspend Elimination
...
|---|---|---|
| **Identity** | Custom branding | `uname -r` -> `6.19.6-Hyperion-2.2.0` |
...
This is a dedicated section because USB peripheral stability is one of the most common sources of silent frustration on Linux. Hyperion v2.2.0 addresses it at every layer.
...
The companion files are distributed as a separate artifact (`hyperion-companion-2.2.0.tar.gz`) attached to the GitHub release. They are **not** baked into the kernel package because:
...
Hyperion v2.2.0 ships with three LSMs compiled in simultaneously. The active LSM is selected by the distro or user via the `security=` kernel command-line parameter.
...
- Kernel headers installed to `/usr/src/linux-headers-6.19.6-Hyperion-2.2.0/`
- Build symlink `/lib/modules/6.19.6-Hyperion-2.2.0/build` always points to the correct headers directory
...
Hyperion v2.2.0 is designed and tested for distribution-agnostic deployment. A single `bzImage` + `initramfs` pair boots and operates correctly on every mainstream Linux distribution.
...
# Should point to /usr/src/linux-headers-6.19.6-Hyperion-2.2.0
...
```bash
# mkinitcpio (Arch)
sudo mkinitcpio -k 6.19.6-Hyperion-2.2.0 \
  -g /boot/initramfs-6.19.6-Hyperion-2.2.0.img
...
sudo dracut --force /boot/initramfs-6.19.6-Hyperion-2.2.0.img \
  6.19.6-Hyperion-2.2.0

# initramfs-tools (Debian/Ubuntu)
sudo update-initramfs -c -k 6.19.6-Hyperion-2.2.0
```
...
# Verify
sudo dkms status
# v4l2loopback/0.12.x, 6.19.6-Hyperion-2.2.0, x86_64: installed
```
...
wget https://github.com/pro-grammer-SD/hyperion/releases/download/v2.2.0/hyperion-companion-2.2.0.tar.gz

# Extract
tar -xzf hyperion-companion-2.2.0.tar.gz
cd hyperion-companion-2.2.0
...
**Hyperion Kernel v2.2.0** · Soumalya Das · 2026
