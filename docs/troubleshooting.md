...
**Symptom:**
```
Error! Bad return status for module build on kernel: 6.19.6-Hyperion-2.0.1
```
...
```bash
readlink /lib/modules/6.19.6-Hyperion-2.0.1/build
# Must point to /usr/src/linux-headers-6.19.6-Hyperion-2.0.1

# Fix if wrong:
sudo ln -sfn /usr/src/linux-headers-6.19.6-Hyperion-2.0.1 \
    /lib/modules/6.19.6-Hyperion-2.0.1/build
```

**Step 3: Try building DKMS module manually**
```bash
sudo dkms build -m nvidia -v 550.54.14 -k 6.19.6-Hyperion-2.0.1 --verbose
```
...
dmesg: foo: version magic '6.19.6 ...' should be '6.19.6-Hyperion-2.0.1 ...'
```
